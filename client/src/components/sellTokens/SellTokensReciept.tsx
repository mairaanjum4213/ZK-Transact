...
const SellTokensReciept: React.FC = () => {
  ...
  const fetchSellToken = async () => {
    try {
      ...
      setSellTokensReceiptData(result.data);
    } catch (error) {
      console.error("Error fetching files:", error);
    }
  };
  
  const handleApproveSellToken = async (id:any) => {
    try {
      const response = await axios.put(`/api/sellToken/approve/${id}`);
      toast.success('Sell token request sent successfully to admin');
      navigate('/sellTokens-approval');

    } catch (error) {
      toast.error('Error sending sell token request');
    }
  };

  const handleButtonClick = () => {
    const sellTokenRequestId = sellTokensRecieptData?._id;
    handleApproveSellToken(sellTokenRequestId);
  };
  return (
    <>
      <BreadCrumb
        parentPageLink="/sellTokens"
        ParentPage="Sell Tokens"
        pageName="Reciept"
        ChildPage="Reciept"
        imageUrl={aboutUs}
      />
      <p className="bg-danger">
        There are two reciept aik jisa user dek ka request bja ga dosri admin sa
        approve hoka jo bna gi user can dwonload only approved one doesri ma
        download waal div urana ha
      ...
      <div className="row gutters">
        <div className="col-xl-12 col-lg-12 col-md-12 col-sm-12 ">
          <div className="text-right ">
            <button
              className="simpleButton1 mt-2 mx-5 px-3"
                          ...
                          <table className="table custom-table     ">
                            ...
                            <tbody>
                              <tr>
                                ...
                                <td className="p-3 customTabletd">
                                  {sellTokensRecieptData.Tokens}{" "}
                                  <span>zkt</span>
                                </td>
                                <td className="p-3 customTabletd">
                                  {sellTokensRecieptData.transactionFee}{" "}
                                </td>
                                <td className="p-3 customTabletd">
                                  {" "}
                                  {
                                    sellTokensRecieptData.localCurrencyAmount
                                  }{" "}
                                </td>
                              </tr>
                            </tbody>
      ...
      <div className=" mb-4  text-center ">
        <button className="btnStyle mt-4" onClick={handleButtonClick}>Request Reciept Approval</button>
      </div>
    </>
  );
};
